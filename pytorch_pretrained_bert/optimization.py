...
def warmup_twofold(x, warmup0, warmup1, lr_discount=0.5):
    # https://www.desmos.com/calculator/fo9zls7nru
    if x < warmup0:
        return x/warmup0
    elif x < warmup1:
        # y = (x + (w_0-x)*d*(1-w_1)/(1-w_0) - w_1) / (w_0 - w_1)
        return (x + (warmup0-x)*lr_discount*(1.-warmup1)/(1.-warmup0) - warmup1) / (warmup0-warmup1)
    else:
        return max(lr_discount*(1.-x)/(1.-warmup0), 0)
